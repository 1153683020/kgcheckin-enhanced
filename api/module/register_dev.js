...
// 生成合法的 32 位小写十六进制设备标识（uuid/imei）。
// 服务器对设备注册接口校验 uuid 合法性，缺失会返回 error_code 20010 “not uuid”。
const genDeviceId = () => nodeCrypto.randomBytes(16).toString('hex');
module.exports = (params, useAxios) => {
  ...
  //IMEI 号（未提供时与 uuid 一致）
  const imei = params?.imei || params.cookie?.KUGOU_API_GUID || genDeviceId();
  //sim 卡号序号
  const imsi = params?.imsi || '';
  //厂商
  const manufacturer = params?.manufacturer || 'Xiaomi';
  //设备uuid（注册接口必填：缺失时生成合法随机设备标识）
  const uuid = params?.uuid  || params.cookie?.KUGOU_API_GUID || genDeviceId();
  //是否有加速度传感器
